...
assert.match(workout, /body\.workout-active \.app\s*\{[^}]*linear-gradient\([^}]*#ffffff env\(safe-area-inset-top\)[^}]*#000000 env\(safe-area-inset-top\)/s);
assert.match(workout, /#exerciseList > \.workout-accordion-card:first-child:not\(\.open\) \.exercise-chip-toggle\s*\{[^}]*padding-top:\s*env\(safe-area-inset-top\)/s);
assert.match(workout, /#exerciseList > \.workout-accordion-card:first-child\.open \.exercise-card-body\s*\{[^}]*padding-top:\s*max\([^}]*env\(safe-area-inset-top\)/s);
